package com.fy.cms.Config.Security;
...
public class RbacAuthorityService {
    ...
    public boolean hasPermission(HttpServletRequest request, Authentication authentication) {
        ...
        if (userInfo instanceof UserDetails) {
            ...
            SysUser sysUserByDataBase=sysUserService.findOneByUserNameOrEmailAndPassword(sysUser.getUsername(),sysUser.getEmail(),sysUser.getPassword());
            List<SysPermission> permissions=sysUserByDataBase.getPermissions();

            //获取资源，前后端分离，所以过滤页面权限，只保留按钮权限
            List<SysPermission> btnPerms = permissions.stream()
                    // 过滤页面权限
                    .filter(permission -> "2".equals(permission.getType()))
                    // 过滤 URL 为空
